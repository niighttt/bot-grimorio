...
TIPOS_MALDICAO = {
    "latim": {
        "nome": "Maldição do Latim Antigo",
        "desc": "Tudo que fala vira latim macarrônico",
        "transformar": lambda txt: txt + "us dominus vobiscum est"
    },
    "caipira": {
        "nome": "Maldição do Capiau",
        "desc": "Vira caipira da roça",
        "transformar": lambda txt: txt.replace("lho", "io").replace("nho", "in").replace("você", "ocê") + " uai sô"
    },
    "oposto": {
        "nome": "Maldição do Contrário",
        "desc": "Fala o oposto do que quer",
        "transformar": lambda txt: txt.replace("sim", "não").replace("bom", "ruim").replace("gosto", "odeio").replace("quero", "não quero")
    },
    "sussurro": {
        "nome": "Maldição do Sussurro",
        "desc": "Tudo sai como spoiler",
        "transformar": lambda txt: f"||{txt}||"
    },
    "emoji": {
        "nome": "Maldição do Emoji",
        "desc": "Cada palavra vira emoji aleatório",
        "transformar": lambda txt: " ".join([random.choice(["💀","👻","🔮","🐸","💨","🤡"]) for _ in txt.split()])
    },
    "gago": {
        "nome": "Maldição do Gago",
        "desc": "Começa a gaguejar",
        "transformar": lambda txt: " ".join([w[0] + "-" + w if len(w) > 2 else w for w in txt.split()])
    },
    "grito": {
        "nome": "Maldição do Berro",
        "desc": "TUDO EM CAPSLOCK",
        "transformar": lambda txt: txt.upper() + "!!!"
    },
    "pirata": {
        "nome": "Maldição do Marujo",
        "desc": "Fala que nem pirata",
        "transformar": lambda txt: txt.replace("você", "marujo").replace("sim", "arr") + " YARRR 🏴‍☠️"
    }
}
...
async def maldicao(ctx, membro: discord.Member, tipo: str = None):
    ...
    maldicoes_ativas[membro.id] = {
        "tipo": tipo,
        "expira": time.time() + 3600 # 1 hora
    }

    mald = TIPOS_MALDICAO[tipo]
    embed = discord.Embed(
        title="💀 MALDIÇÃO LANÇADA 💀",
        description=f"{membro.mention} foi atingido pela **{mald['nome']}**!",
        color=0x8B0000
    )
    embed.add_field(name="Efeito", value=mald['desc'])
    embed.add_field(name="Duração", value="1 hora")
    embed.set_footer(text="Use /desfazer para remover antes do tempo")
    await ctx.send(embed=embed)
...
async def desfazer(ctx, membro: discord.Member):
    if membro.id in maldicoes_ativas:
        ...
        await ctx.send(f"✨ {membro.mention} foi libertado da maldição!")
    else:
        await ctx.send("Esse usuário não está amaldiçoado.")

@bot.command()
async def maldicoes(ctx):
    lista = "\n".join([f"`{k}` - {v['nome']}" for k, v in TIPOS_MALDICAO.items()])
    await ctx.send(f"**Lista de Maldições Disponíveis:**\n{lista}\n\nUse: `/maldicao @user tipo`")
...
async def on_message(message):
    ...
    # Checa se o autor tá amaldiçoado
    if message.author.id in maldicoes_ativas:
        ...
        if time.time() > dados["expira"]:
            ...
            tipo_mald = dados["tipo"]
            texto_novo = TIPOS_MALDICAO[tipo_mald]["transformar"](message.content)

            webhook = await message.channel.create_webhook(name=message.author.display_name)
            await webhook.send(
                texto_novo,
                username=f"{message.author.display_name} Amaldiçoado",
                avatar_url=message.author.avatar.url
            )
            await webhook.delete()
            return # Não processa comandos pra não bugar
